...
bool obtenerConfiguracion(int* filas, int* columnas, int* cantidadMinas)
{
    ...
    while (fgets(linea, sizeof(linea), archivo))
    {
        ...
        if (strncmp(linea, "Tamanio=", 8) == 0)
        {
            ...
            {
                printf("\nTamanio fuera de los limites (8 a 32), usando configuracion por defecto\n");
                fclose(archivo);
                return false;
            }
            leyoTamanio = true;
...
int jugarPartida(const char* nombre_usuario, int continuar)
{
    ...
    int screen_height = (filas + 1) * PIXEL_POR_LADO * pixel_size;
    ...
    Estadistica est = {filas, columnas, cantidadMinas, segundos, resultado};
